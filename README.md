...
* inn [строка из 9 или 10 символов, опционален] - ИНН организации
* ogrn [строка из 13 символов, опционален] - ОГРН организации
* okpo [строка из 8 символов, опционален] - ОГРН организации

В запросе должен быть передан по крайней мере один идентификатор (ИНН, ОГРН или ОКПО)
В случае передачи двух (например, ИНН и ОГРН) или трех (ИНН, ОГРН и ОКПО) поиск
будет производиться по соответствию всех переданных идентификаторов.
...
`?inn=7714611569&&subsidiary=false`

### Ответ

```json
{
    "found" : "<количество найденных организаций>",
    "organizations": [
        {
            "inn" : "<ИНН>",
            "ogrn" : "<ОГРН>",
            "okpo" : "<ОКПО>",
            "bsn" : "уникальный идентификатор организации в системе",
            "address" : "<Адрес>",
...
Ответ содержит количество найденных компаний (поле found) и набор карточек (поле organizations).
...
наименование организации. В случае, если не найдено ни одной организации (значене found равно 0),
поле organizations представляет собой пустой массив.
...
### Пример №3: Поиск организации по ОГРН и ОКПО с учетом филиалов
